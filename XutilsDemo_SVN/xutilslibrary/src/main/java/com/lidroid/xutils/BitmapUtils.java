...
public class BitmapUtils implements TaskHandler {
    ...
    public <T extends View> void display(T container, String uri, BitmapDisplayConfig displayConfig, BitmapLoadCallBack<T> callBack) {
        ...
        // find bitmap from mem cache.先从内存缓存中读取该bitmap
        Bitmap bitmap = globalConfig.getBitmapCache().getBitmapFromMemCache(uri, displayConfig);

        if (bitmap != null) {
            //如果内存缓存直接有该对应的uri的bitmap 直接结束
    ...
    public class BitmapLoadTask<T extends View> extends PriorityAsyncTask<Object, Object, Bitmap> {
        private final String uri;//url连接
        private final WeakReference<T> containerReference;//相关软引用 为何要对
        private final BitmapLoadCallBack<T> callBack;//回调函数
        private final BitmapDisplayConfig displayConfig;//相关的配置类
